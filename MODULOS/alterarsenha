#!/bin/bash
tput setaf 7 ; tput setab 4 ; tput bold ; printf '%35s%s%-10s\n' "เปลี่ยนรหัสผ่านผู้ใช้" ; tput sgr0
echo ""
echo -e "\033[1;33mรายชื่อผู้ใช้และรหัสผ่าน: \033[0m"
echo""
...
then
	...
	then
		...
		if [[ $sizepass -lt 4 ]]
		then
			tput setaf 7 ; tput setab 1 ; tput bold ; echo "" ; echo "รหัสผ่านว่างเปล่าหรือไม่ถูกต้อง!  ใช้อักขระอย่างน้อย 4 ตัว" ; echo "" ; tput sgr0
			exit 1
		else
			...
			then
				...
				tput setaf 7 ; tput setab 1 ; tput bold ; echo "" ; echo "รหัสผ่านผู้ใช้ $user ได้เปลี่ยนเป็น: $password" ; echo "" ; tput sgr0
				echo "$password" > /etc/SSHPlus/senha/$user
				exit 1
			else
				echo ""
				tput setaf 7 ; tput setab 4 ; tput bold ; echo "ผู้ใช้เข้าสู่ระบบ  กำลังตัดการเชื่อมต่อ..." ; tput sgr0
				pkill -f $user
				echo "$user:$password" | chpasswd
		...
		tput setaf 7 ; tput setab 4 ; tput bold ; echo "" ; echo "บัญชีผู้ใช้ $user ไม่มีอยู่!" ; echo "" ; tput sgr0
	fi
fi
